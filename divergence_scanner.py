...
class AlertFormatter:
    ...
    def format_alert(alert) -> str:
        """Format a complete alert signal for Telegram"""
        ...
        # Momentum emojis
        rsi_emoji = "✅" if alert.momentum.rsi_confirmed else "⏳"
        price_emoji = "✅" if alert.momentum.price_confirmed else "⏳"
        vol_emoji = "✅" if alert.momentum.volume_confirmed else "⏳"
        
        # TradingView
        tv_data = alert.tv_data
        if tv_data and 'summary' in tv_data:
            tv_rec = tv_data['summary'].get('RECOMMENDATION', 'NEUTRAL')
            if tv_rec == 'STRONG_BUY' or tv_rec == 'BUY':
                tv_emoji = "🟢"
            elif tv_rec == 'STRONG_SELL' or tv_rec == 'SELL':
                tv_emoji = "🔴"
            else:
                tv_emoji = "⚪"
        else:
        ...
        if is_bull:
            stop = min(div.swing1.price, div.swing2.price) * 0.99
            target = entry * 1.04
            trade = "LONG"
        else:
            stop = max(div.swing1.price, div.swing2.price) * 1.01
            target = entry * 0.96
            trade = "SHORT"
        
        # Candle close time
        candle_time = alert.candle_close_time
        candle_time_str = format_sl_time(candle_time)
        
        # Build message
        msg = f"""{strength_emoji} {strength_label} SIGNAL - {direction}
...
{vol_emoji} Volume: {alert.momentum.volume_direction}
{'━'*28}
