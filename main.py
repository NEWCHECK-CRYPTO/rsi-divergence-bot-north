"""
RSI Divergence Alert Bot - V6
- Bybit Exchange (no geo-restrictions)
- 100 Top Coins
- TradingView RSI validation
- Trade Journal
"""
...
class HealthHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        html = f"""<!DOCTYPE html><html><head><title>RSI Bot V6</title>
...
async def debug_scan(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        msg += f"• Min Dist: {MIN_SWING_DISTANCE}\n"
        ...
        if swing_lows:
            ...
            for sl in swing_lows[-3:]:
                msg += f"• ${sl.price:,.4f} RSI:{sl.rsi:.1f}\n"
        
        if swing_highs:
            msg += f"\n*Swing Highs:*\n"
            for sh in swing_highs[-3:]:
                msg += f"• ${sh.price:,.4f} RSI:{sh.rsi:.1f}\n"
        ...
        if len(valid_lows) >= 2:
            ...
            price_chg = ((p2.price - p1.price) / p1.price) * 100
            rsi_chg = p2.rsi - p1.rsi
            
            if p2.price < p1.price and p2.rsi > p1.rsi:
                msg += f"\n✅ *BULLISH DIVERGENCE!*\n"
                msg += f"Price: {price_chg:+.2f}% RSI: {rsi_chg:+.1f}\n"
            else:
                msg += f"\n❌ No bullish (Price:{price_chg:+.1f}% RSI:{rsi_chg:+.1f})\n"
        ...
        if len(valid_highs) >= 2:
            ...
            if p2.price > p1.price and p2.rsi < p1.rsi:
                ...
                msg += f"\n❌ No bearish (Price:{price_chg:+.1f}% RSI:{rsi_chg:+.1f})\n"
...
async def scheduled_scan(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        for chat_id, prefs in subscribers.items():
            ...
            for alert in user_alerts[:5]:  # Max 5 per scan
                try:
...
def main():
    ...
    logger.info(f"[{format_sl_time()}] 🤖 Bot V6 ({EXCHANGE.upper()}) starting...")
